...
public class EMFModelPart extends ModelPart  {
    private final List<EMFCuboid> cuboids = new ArrayList<>();
    private final Map<String, EMFModelPart> children = new HashMap<>();


    public final EMFPartData selfModelData;
    public final ArrayList<EMFPartData> parentModelData;
    public final EMFModelPart parent;
       ...
       // if("left_rein2".equals(selfModelData.id) && new Random().nextInt(100) == 1) System.out.println("rein visible? " + visible +", "+hidden+","+visibilityIsOveridden);
        ...
        if (this.visible){// && xScale!= 0 && yScale != 0 && zScale != 0){// && scaleNotZero) {
            //if (!this.cuboids.isEmpty() || !this.children.isEmpty()) {
                ...
                Iterator<EMFModelPart> var9 = this.children.values().iterator();

                while(var9.hasNext()) {
                    EMFModelPart modelPart = var9.next();
...
//    }
    ...
    public EMFModelPart(EMFModelPart parent, int parentNumber,
                        EMFPartData EMFmodelData,
                        ArrayList<EMFPartData> parentEMFmodelData,
...
//        }
        ...
        for (EMFPartData sub : selfModelData.submodels) {
            //seems like i need to alter parentcount 1's pivots
            ArrayList<EMFPartData> hold = new ArrayList<>(parentEMFmodelData);
            hold.add(selfModelData);
            while(children.containsKey(sub.id)){
                sub.id = sub.id+"-";
            }
            children.put(sub.id, new EMFModelPart(this,parentNumber + 1, sub, hold,sendToFirstChild,null,null,this.thisModel));
        }
        boolean emptyChildren = true;
        for (EMFModelPart child:
             children.values()) {
                ...
                for (EMFBoxData box :
                    ...
                    if (box.textureOffset.length == 2) {
                        ...
                        cube = new EMFCuboid(selfModelData,
                                ...
                                selfModelData.textureSize[0], selfModelData.textureSize[1],
                                selfModelData.mirrorTexture.contains("u"),selfModelData.mirrorTexture.contains("v"));//selfModelData.invertAxis);
                    } else {
...
//    }
    ...
    public Object2ReferenceOpenHashMap<String, EMFModelPart> getAllParts(){
